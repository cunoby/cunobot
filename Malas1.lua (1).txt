  ...
  function WindowAPI:AddMainTab(MainName, IsMainOpen)
      ...
      function MainTabAPI:AddSection(Title, OpenSection)
          ...
          function Item:AddPopUpLive(Config)
            ...
            -- 2. Buat GUI Panel Mengambang
            local HUDMain = Custom:Create("Frame", {
                Size = UDim2.new(0, 230, 0, 280), Position = UDim2.new(1, -260, 0.5, -140),
                BackgroundColor3 = Color3.fromRGB(15, 15, 15), BackgroundTransparency = 0.2,
                BorderSizePixel = 0, Active = true, Draggable = true, Visible = false,
                Parent = SpeedHubXGui -- Memasukkan langsung ke UI Utama Malas.lua
            })
            Custom:Create("UICorner", {CornerRadius = UDim.new(0, 8)}, HUDMain)
            Custom:Create("UIStroke", {Color = Custom.ColorRGB, Thickness = 1.6}, HUDMain)

            Custom:Create("TextLabel", {
                Size = UDim2.new(1, 0, 0, 30), BackgroundTransparency = 1,
                Text = "🛒 " .. PanelTitle, TextColor3 = Color3.fromRGB(255, 255, 255),
                Font = Enum.Font.GothamBold, TextSize = 14, Parent = HUDMain
            })

            local CloseBtn = Custom:Create("TextButton", {
                Size = UDim2.new(0, 25, 0, 25), Position = UDim2.new(1, -30, 0, 2),
                BackgroundTransparency = 1, Text = "X", TextColor3 = Color3.fromRGB(255, 255, 255),
                Font = Enum.Font.GothamBold, TextSize = 14, Parent = HUDMain
            })
            CloseBtn.Activated:Connect(function() HUDMain.Visible = false end)

            Custom:Create("Frame", {
                Size = UDim2.new(1, -20, 0, 2), Position = UDim2.new(0, 10, 0, 30),
                BackgroundColor3 = Custom.ColorRGB, BorderSizePixel = 0, Parent = HUDMain
            })

            local HUDScroll = Custom:Create("ScrollingFrame", {
                Size = UDim2.new(1, 0, 1, -40), Position = UDim2.new(0, 0, 0, 35),
                BackgroundTransparency = 1, BorderSizePixel = 0, ScrollBarThickness = 4,
                ScrollBarImageColor3 = Custom.ColorRGB, Active = true, Parent = HUDMain
            })

            local HUDContent = Custom:Create("TextLabel", {
                ...
                TextYAlignment = Enum.TextYAlignment.Top, Parent = HUDScroll
            })
            ...
            function Funcs_PopUp:Set(Text, Lines)
                Lines = Lines or 3
                local textHeight = 15 + (Lines * 15)
                HUDContent.Size = UDim2.new(1, -10, 0, textHeight)
                HUDScroll.CanvasSize = UDim2.new(0, 0, 0, textHeight + 10)
                HUDContent.Text = Text
            end
