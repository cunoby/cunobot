...
local Age500MinAge   = 55
local Age500MaxAge   = 500
...
pcall(function()
    if isfile and readfile and isfile(SaveFileName) then
        ...
        if data then
            ...
            SavedData.Age500 = data.Age500 or {}
...
local Age500MinAge   = SavedData.Input.AgeMin
local Age500MaxAge   = SavedData.Input.AgeMax
...
local function RegisterPet(uuid, isFav)
    ...
    -- [PERBAIKAN]: Blokir pet bahan yang sudah Age 500 agar tidak masuk Dropdown
    if not isFav and umur >= Age500MaxAge then return end
    ...
    for _, petBahan in ipairs(PetBahan) do
        ...
        for _, id in ipairs(BahanDiKebun) do
            if id == petBahan.Id then
                inKebun = true
                if AmbilUmurDiKebun(id) < Age500MaxAge then sisa = sisa + 1 end
                break
            end
        end
        if not inKebun then
            for _, p in ipairs(NonFav) do
                if p.Id == petBahan.Id then
                    if p.Umur < Age500MaxAge then sisa = sisa + 1 end
                    break
                end
            end
        end
...
local TabLeveling = Window:AddMainTab("Auto Leveling", false)
...
-- SECTION 3: AGE 500
local SecAge500 = TabLeveling:AddSection("Team Age 500 Settings", false)
local DropAge500 = SecAge500:AddDropdown({
    Title = "Pilih Team Age 500", Content = "Pilih 1 atau lebih", Multi = true, Options = {"Kosong"}, Default = SavedData.Age500,
    Callback = function(Options) 
        UpdateMultiSelectState(FavPet, Options, PetTeamAge500) 
        if not IsRefreshingUI then SavedData.Age500 = Options SaveSettings() end
    end
})
SecAge500:AddInput({ Title = "Minimum Age", Content = "Bypass Gajah", Default = tostring(SavedData.Input.AgeMin), Callback = function(Text) Age500MinAge = tonumber(Text) or 55; SavedData.Input.AgeMin = Age500MinAge; SaveSettings() end })
SecAge500:AddInput({ Title = "Maximum Age", Content = "Target panen", Default = tostring(SavedData.Input.AgeMax), Callback = function(Text) Age500MaxAge = tonumber(Text) or 500; SavedData.Input.AgeMax = Age500MaxAge; SaveSettings() end })
...
local function UpdateSemuaDropdown(paksaRefresh)
    ...
    if DropAge500 then DropAge500:Refresh(daftarFav, BersihkanGaib(DropAge500.Value, daftarFav)) end
    ...
    if DropAge500 then UpdateMultiSelectState(FavPet, DropAge500.Value, PetTeamAge500) end
    ...
    while task.wait(0.5) do 
        if AutoElephantOn then 
            ...
            elseif FaseFarming == "PUSH_LEVELING" then
                local semuaSiapAge500 = true
                for _, id in ipairs(BahanDiKebun) do if AmbilUmurDiKebun(id) < Age500MinAge then semuaSiapAge500 = false break end end
                if semuaSiapAge500 then
                    LogPesan("[Fase] Push Selesai, masuk Age 500")
                    for _, petTeam in ipairs(PetTeamLeveling) do PickupPet(petTeam.Id) task.wait(0.4) end
                    for _, petTeam in ipairs(PetTeamAge500) do PlacePet(petTeam.Id) task.wait(0.4) end
                    FaseFarming = "MENUJU_500"
                end

            elseif FaseFarming == "MENUJU_500" then
                local semuaSudahMax = true
                for _, id in ipairs(BahanDiKebun) do if AmbilUmurDiKebun(id) < Age500MaxAge then semuaSudahMax = false break end end
                if semuaSudahMax then
                    ...
                    for _, petTeam in ipairs(PetTeamAge500) do PickupPet(petTeam.Id) task.wait(0.4) end
                    FaseFarming = "TANAM" 
                end
            end
        end
    ...
    while task.wait(0.03) do -- Respon super cepat
        ...
        local function IsPetActiveInFSM(petId)
            if AutoElephantOn then
                ...
                for _, p in ipairs(PetTeamAge500) do if p.Id == petId then isFSMTarget = true end end
                if not isFSMTarget then 
                    return false 
                end 
            end
            return true 
